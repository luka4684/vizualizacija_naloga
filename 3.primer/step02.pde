...
void setupStandings() {
  ...
  //println("minDateIndex: "+minDateIndex+", maxDateIndex: "+maxDateIndex);
  for (int i = minDateIndex; i <= maxDateIndex; i++) {
    //println("dateStamp for "+i+": "+dateStamp[i-1]);
    String[] lines = acquireStandings(dateStamp[i-1]);
...
void setupStandings() {
  ...
  println(lines.length+" "+teamCount);
  int expectedCount = (maxDateIndex - minDateIndex) + 1;
  println(maxDateIndex+" "+minDateIndex);
